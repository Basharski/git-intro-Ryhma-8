...
export const createPatientProLink = async (proId, patientId, permissionsJson) => {
...
export const updateLinkPermissions = async (proId, patientId, permissionsJson) => {
...
export const removePatientProLink = async (proId, patientId) => {
  const [result] = await promisePool.execute(
    'DELETE FROM patient_pro_links WHERE patient_id = ? AND pro_id = ?',
    [patientId, proId]
  );
