...
async function handleDeleteAccount() {
  const firstConfirm = confirm(
    'Oletko varma, että haluat poistaa tilisi? Tätä toimintoa ei voi peruuttaa ja kaikki datasi häviää.',
  );

  if (firstConfirm) {
    const secondConfirm = prompt(
      'Kirjoita POISTA vahvistaaksesi tilin lopullisen poistamisen: ',
    );

    if (secondConfirm === 'POISTA') {
      try {
        await deleteAccount();
        showSnackbar('Tilisi on poistettu. Sinut kirjataan ulos.');
        logout();
      } catch (err) {
        showSnackbar('Virhe tilin poistamisessa.');
      }
    }
  }
}
  ...
  ?.addEventListener('click', async () => {
    const profileData = {
      ...
      weight:
        ...
        undefined,
      age:
        parseInt(document.getElementById('profile-age').value, 10) || undefined,
    };
